<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/MasterPage.master" CodeFile="utilization.aspx.cs" Inherits="E_Utilization" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
            ...
            $.ajax(
                {
                    ...
                    url: '/api/mpx/v1/output/e_util.aspx/SetGoogleData',
        ...
        function drawVisualization(dataValues) {
            ...
            data.addColumn({ type: 'string', role: 'annotation', label: 'LABOR ASSIGNED' });
            ...
            for (var i = 0; i < dataValues.length; i++) {
                ...
                if (Number(dataValues[i][2]) != null) {
                    var setup = Number(dataValues[i][2]);
                } else {
                    var setup = 0;
                }
                if (Number(dataValues[i][3]) != null) {
                    var wait = Number(dataValues[i][3]);
                } else {
                    var wait = 0;
                }
                ...
                if (Number(dataValues[i][5]) != null) {
                    var repair = Number(dataValues[i][5]);
                } else {
                    var repair = 0;
                }
                if (Number(dataValues[i][7]) != null) {
                    var time = Number(dataValues[i][7]);
                } else {
                    var time = 0;
                }


                total = setup + wait + run + repair;

                var name = dataValues[i][0];
                var scenario = dataValues[i][1];
                var labor = dataValues[i][6];
                var concat;
                ...
                if (total != 0) {
                    data.addRow([concat, name, scenario, myFunction(name, scenario, repair, wait, setup, run, total, time, labor), repair, wait, setup, run, total, null, time, labor, null]);
                }
            ...
            if (data.getNumberOfRows() != 0) {
                ...
                for (var i = 0; i < numNames; i++) {
                    ...
                    var nameMax = tempView.getColumnRange(8);
                ...
                data.sort([9, { column: 8, desc: true }]);


                var maxValue = data.getValue(0, 8)
                ...
                myTable = new google.visualization.ChartWrapper({
                    ...
                    'view': { "columns": [1, 2, 11, 10, 8, 4, 5, 6, 7] }
        ...
        function myFunction(myName, myScenario, myRepair, myWait, mySetup, myRun, myTotal, myTime, myLabor) {
            ...
            if (myWait != 0) {
                array.push('<div><span style="">WAIT FOR LABOR: </span><span style="font-weight:900;"> ' + myWait.toFixed(1) + '%</span></div>');

            }


            if (myRepair != 0) {
                array.push('<div><span style="">REPAIR: </span><span style="font-weight:900;"> ' + myRepair.toFixed(1) + '%</span>');
            }
            ...
            var myreturnString = '<div style="padding:10px; display:flex;flex-direction:column;font-size:12px;color:#555;">' +
                '<span style="font-weight:900; font-size:16px;">' + myName + ', ' + myTotal.toFixed(1) + '%</span>' +
                '<span style="font-weight:900; font-size:14px;">' + myLabor + '</span>' +
        ...
        function googleToggleAxis() {
            if (myChart.getOption('isStacked') == "true" & percent) {
                for (var i = 0; i < data.getNumberOfRows(); i++) {
                    ...
                    var myTime = data.getValue(i, 10);
                    var myRepair = data.getValue(i, 4) / 100 * myTime;
                    var myWait = data.getValue(i, 5) / 100 * myTime;
                    var mySetup = data.getValue(i, 6) / 100 * myTime;
                    var myRun = data.getValue(i, 7) / 100 * myTime;
                    var myTotal = data.getValue(i, 8);
                    if (myTotal < 100) {
                        myIdle = (100 - myTotal) / 100 * myTime;
                    } else {
                        myIdle = null;
                    }
                    myTotal = data.getValue(i, 8) / 100 * myTime;

                    data.setValue(i, 4, myRepair);
                    data.setValue(i, 5, myWait);
                    data.setValue(i, 6, mySetup);
                    data.setValue(i, 7, myRun);
                    data.setValue(i, 8, myTotal);
                    data.setValue(i, 12, myIdle);
                ...
                for (var i = 0; i < data.getNumberOfRows(); i++) {
                    ...
                    var mySetup = data.getValue(i, 6) * 100 / myTime;
                    var myRun = data.getValue(i, 7) * 100 / myTime;
                    var myTotal = data.getValue(i, 8) * 100 / myTime;
                ...
                $.ajax(
                    {
                        ...
                        dataType: 'json',
                        contentType: 'application/json',
                        url: '/api/mpx/v1/output/e_util.aspx/GetMaxUtil',
